...
from pm_command_support import current_task_cfg, emit_json, task_summary_text
...
def _get_task_record(api: Any, *, task_guid: str, task_id: str, include_completed: bool) -> dict[str, Any]:
    if task_guid:
        return api.get_task_record_by_guid(task_guid)
    return api.get_task_record(task_id, include_completed=include_completed)


def _require_task_guid(task: dict[str, Any], task_ref: str) -> str:
    guid = str(task.get("guid") or "").strip()
    if not guid:
        raise SystemExit(f"task missing guid: {task_ref}")
    return guid


def cmd_create(api: Any, args: argparse.Namespace) -> int:
...
def cmd_get(api: Any, args: argparse.Namespace) -> int:
    task = _get_task_record(api, task_guid=args.task_guid, task_id=args.task_id, include_completed=args.include_completed)
    guid = str(task.get("guid") or "").strip()
...
def cmd_comment(api: Any, args: argparse.Namespace) -> int:
    task = _get_task_record(api, task_guid=args.task_guid, task_id=args.task_id, include_completed=args.include_completed)
    guid = _require_task_guid(task, args.task_id or args.task_guid)
...
def cmd_complete(api: Any, args: argparse.Namespace) -> int:
    task = _get_task_record(api, task_guid=args.task_guid, task_id=args.task_id, include_completed=args.include_completed)
    guid = _require_task_guid(task, args.task_id or args.task_guid)
    ...
    if upload_result.get("status") == "authorization_required":
        ...
        upload_result["commit_url"] = args.commit_url.strip() or (
            "" if args.skip_head_commit_url else api.current_head_commit_url(args.repo_root)
        )
...
def cmd_update_description(api: Any, args: argparse.Namespace) -> int:
    task = _get_task_record(api, task_guid=args.task_guid, task_id=args.task_id, include_completed=args.include_completed)
    guid = _require_task_guid(task, args.task_id or args.task_guid)
...
def cmd_list(api: Any, args: argparse.Namespace) -> int:
...
def cmd_normalize_titles(api: Any, args: argparse.Namespace) -> int:
    result = api.normalize_task_titles(include_completed=args.include_completed)
    api.refresh_context_cache()
    return _emit(result)


def cmd_search(api: Any, args: argparse.Namespace) -> int:
    ...
    matches.sort(
        key=lambda item: api.extract_task_number(str(item.get("normalized_summary") or item.get("summary") or "")),
        reverse=True,
    )
    if args.limit:
...
def _selected_tasklists(api: Any, args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    return [api.ensure_tasklist()]


def _resolve_backfill_current_user_id(api: Any, auth: dict[str, Any], tasklists_by_guid: dict[str, dict[str, Any]]) -> tuple[str, str]:
...
def _collect_backfill_candidates(
    ...
    return candidates, skipped_by_reason, len(scanned_task_guids)
...
def cmd_backfill_assignees(api: Any, args: argparse.Namespace) -> int:
    selected_tasklists = _selected_tasklists(api, args)
    ...
    for item in selected_tasklists:
        ...
        guid = str(item.get("guid") or "").strip()
        if guid:
            tasklists_by_guid[guid] = item

    current_user_id, current_user_source = _resolve_backfill_current_user_id(api, auth, tasklists_by_guid)
    ...
    desired_members = resolve_default_task_members(task_config=current_task_cfg(api), current_user_id=current_user_id)
    candidates, skipped_by_reason, scanned_task_count = _collect_backfill_candidates(
        api,
        args,
        tasklists_by_guid,
        current_user_id,
...
def cmd_attachments(api: Any, args: argparse.Namespace) -> int:
    task = _get_task_record(api, task_guid=args.task_guid, task_id=args.task_id, include_completed=args.include_completed)
...
def cmd_upload_attachments(api: Any, args: argparse.Namespace) -> int:
    task = _get_task_record(api, task_guid=args.task_guid, task_id=args.task_id, include_completed=args.include_completed)
