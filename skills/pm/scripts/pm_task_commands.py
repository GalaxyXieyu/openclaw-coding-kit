...
def build_task_command_handlers(api: Any) -> dict[str, CommandHandler]:
    ...
    def cmd_backfill_assignees(args: argparse.Namespace) -> int:
        ...
        def list_tasklist_rows(tasklist_guid: str, *, completed: bool) -> list[dict[str, Any]]:
            page_token = ""
            rows: list[dict[str, Any]] = []
            seen: set[str] = set()
            while True:
        ...
        if args.all_visible_tasklists:
            selected_tasklists = list_visible_tasklists()
        elif explicit_tasklist_guids:
            visible_index = {
                str(item.get("guid") or "").strip(): item
                for item in list_visible_tasklists()
                if isinstance(item, dict) and str(item.get("guid") or "").strip()
            }
            selected_tasklists = [visible_index.get(guid) or {"guid": guid, "name": guid} for guid in explicit_tasklist_guids]
        else:
            selected_tasklists = [api.ensure_tasklist()]
        ...
        def resolve_current_user_id() -> tuple[str, str]:
            ...
            default_owner = str(
                (((api.ensure_tasklist().get("owner") or {}) if isinstance(api.ensure_tasklist().get("owner"), dict) else {}).get("id") or "")
            ).strip()
            if default_owner:
        ...
        current_user_id, current_user_source = resolve_current_user_id()
        ...
        for tasklist_guid, tasklist in tasklists_by_guid.items():
            task_rows = list_tasklist_rows(tasklist_guid, completed=False)
            if not args.open_only:
                task_rows += list_tasklist_rows(tasklist_guid, completed=True)
    ...
    return {
        ...
        "search": cmd_search,
        "backfill_assignees": cmd_backfill_assignees,
