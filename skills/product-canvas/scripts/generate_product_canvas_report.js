...
function main() {
  ...
  const title = args["title"] || "Product Canvas Review Report";
  ...
  if (bugRows.length === 0) {
    ...
    lines.push("| defect_id | severity | module | case_id | title | status | evidence_path |");
    lines.push("|---|---|---|---|---|---|---|");
    for (const row of bugRows) {
      lines.push(
        `| ${safe(row.defect_id)} | ${safe(row.severity)} | ${safe(row.module)} | ${safe(row.case_id)} | ${safe(row.title)} | ${safe(row.status)} | ${safe(row.evidence_path)} |`
      );
    }
    lines.push("");
  }
  ...
  lines.push("## 4. 全量结果");
  lines.push("");
  lines.push("| case_id | priority | result | failure_type | defect_id | evidence_path |");
  lines.push("|---|---|---|---|---|---|");
  for (const item of cases) {
    lines.push(
      `| ${safe(item.caseId)} | ${safe(item.priority)} | ${safe(item.result)} | ${safe(item.failureType)} | ${safe(item.defectId)} | ${safe(item.evidencePath || item.screenshotPath)} |`
    );
  }
  lines.push("");
  ...
  console.log(`generated product-canvas report: ${outputPath}`);
}

try {
  main();
} catch (err) {
  console.error(`[generate_product_canvas_report] ${err.message}`);
  process.exit(1);
}
